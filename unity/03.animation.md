...
2. 而root motion动画中, unity会依据动画文件里记录的**绝对position, rotation和scale(注意scale)**计算出游戏对象在上一帧的相对postion和相对rotation, 这个在代码里记录为**`animator.deltaPosition与animator.deltaRotation`**, 当播放到下一个循环的开头时, 动画的position与rotation并不会被重置为初始值
3. 或者说, root motion会让游戏对象乘以TRS矩阵
4. 动画文件会直接修改每一帧里游戏对象的坐标值和角度, 而root motion:
...
1. 控制多个动画的Blend融合: 
   1. 第1列motion
   2. 第2列Threshold权重信息
   3. 第3列动画播放speeds, 比如可以通过新加一个run动画的并且把speed调整为2.0倍, 从而冒充rush
...
3. Automate Thresholds: 是否自动分配blend用的threshold信息
   ...
   2. 
4. Compute Thresholds: 取消Automate Thresholds后出现.
5. Adjust Timescale: 
