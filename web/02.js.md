...
3. prototype最主要的用途是存储function (稳定)，而field (易变)则适合存储在obj自身上



##### 2 [js design regrets](https://www.zjl.me/2020/11/14/javascript-%E7%9A%84%E8%AE%BE%E8%AE%A1%E5%A4%B1%E8%AF%AF/)

1. ==运算：试图先转换类型后比较，不具备可传递性，所以被弃用。推荐显示转换类型后使用===比较
2. 使用表现更加稳定的undefined代替null
...
typeof NaN === 'number'		// NaN是js中唯一一个不等于自身的值，标准规定很多不同的二进制值都可以当作NaN; 使用isNaN()或Number.isNaN()，这俩不等价
...
4. push与unshift都是长的，pop与shift都是短的
5. sort = 原地排序 + toString()后排序 (糟粕)
...
1. js除this之外的所有变量都是静态绑定的，而this则是动态绑定的。这经常是混乱的根源，所以优选module pattern这种去this化的方案
2. 普通函数
